...
def sort_ephemerides_by_object_df(data: pd.DataFrame):
    ...
    for obj in unique_objects:
        ...
        # Prefer numeric degree coordinates to avoid sexagesimal strings
        x      = obj_data.get("RA", obj_data["RA"]).tolist()
        y      = obj_data.get("DEC", obj_data["DEC"]).tolist()
...
def match_ephemeris_by_site(combined_df, recent_obs_df,
    ...
    def _coerce_ra_deg_series(s):
        """Robustly coerce RA to degrees and wrap into [0,360). Auto-convert hours->deg."""
        a = pd.to_numeric(s, errors='coerce').astype(float)
        finite = a[np.isfinite(a)]
        if len(finite) > 0:
            mn, mx = float(np.nanmin(finite)), float(np.nanmax(finite))
            # looks like hours? (0..24.x and no >24)
            if mn >= -0.2 and mx <= 24.2 and (mx - mn) >= 1.0:
                a = a * 15.0
    ...
    for site_key, grp_obs in _tqdm(site_iters, desc="Sites", unit="site"):
        ...
        for obj_name, g in _tqdm(groups, desc=f"Objs@{site_key}", unit="obj", leave=False):
            ...
            # for j_local, j_obs in _tqdm(enumerate(ok_obs_idx), total=len(ok_obs_idx), desc="Obs", leave=False):
