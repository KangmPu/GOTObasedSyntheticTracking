...
class Client:
    ...
    def _legacy_single_image(self, remote_username: str) -> str:
        ...
        if self.server == "monash":
            ...
            return f"/mnt3/data/public/goto/commissioning/pipeline/{self.date}/final/{self.runid}_{self.camera}{self.median}.fits"
        elif self.server == "warwick":
            if not (self.date and self.runid and self.camera):
                raise ValueError("Need --date/--runid/--camera or provide --images/--image.")
            return f"/export/gotodata2/gotophoto/storage/pipeline/{self.date}/final/{self.runid}_{self.camera}.fits"
        else:
    ...
    @staticmethod
    def _make_gif_from_pngs(png_paths: List[str], gif_path: str, duration: float = 0.2, loop: int = 0):
        ...
        try:
            ...
            try:
                ...
                imgs[0].save(gif_path, save_all=True, append_images=imgs[1:],
                             duration=int(duration*1000), loop=loop, disposal=2)
                return
            except Exception as e2:
                raise RuntimeError(f"Failed to create GIF with imageio and Pillow: {e}; {e2}")
    ...
    def client(self) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            if downloaded:
                self._make_gif_from_pngs(downloaded, self.gif_out,
                                         duration=self.gif_duration, loop=self.gif_loop)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Remote batch thumbnail client: download PNGs, stitch GIF, optionally remove PNGs.")
    parser.add_argument("--server", default="monash", type=str, help='Set server to "monash" or "warwick"')
    parser.add_argument("--verbose", action="store_true", help="Turn on logging and verbosity")
    parser.add_argument("--display", default=True, help="Display info about the generated GIF")

    # legacy parameters (still usable for single-image fallback)
    parser.add_argument("--date", default=None, type=str, help="Night ID for observation")
    parser.add_argument("--runid", default=None, type=str, help="Image ID for observation")
    parser.add_argument("--camera", default=None, type=str, help='UT ID. Give as "UT?"')
    parser.add_argument("--median", default=True, help="Use median-combined images in legacy mode")

    # batch inputs
    parser.add_argument("--images", nargs="+", default=None, help="One or more full remote FITS paths")
    parser.add_argument("--image", default=None, type=str, help="(Deprecated) Single remote FITS path")

    parser.add_argument("--ra", required=True, type=float, help="RA of center (deg)")
    parser.add_argument("--dec", required=True, type=float, help="DEC of center (deg)")
    parser.add_argument("--size", required=True, type=int, help="Box size in pixels")

    parser.add_argument("--outpath", default="~/out", type=str, help="Remote out directory on server (absolute or ~)")
    parser.add_argument("--local_temp", default=".", type=str, help="Local directory to save thumbnails and GIF")

    parser.add_argument("--loose-wcs", action="store_true",
                        help="Pass --loose-wcs to the server to relax WCS handling. Default off.")

    # GIF options
    parser.add_argument("--gif-out", default=None, type=str, help="Local output GIF path; default <local_temp>/thumbs_<timestamp>.gif")
    parser.add_argument("--gif-duration", default=0.2, type=float, help="Per-frame duration (seconds) for the GIF (default 0.2s)")
    parser.add_argument("--gif-loop", default=0, type=int, help="GIF loop count (0=infinite; 1=play once; 2=play twice; ...)")
    parser.add_argument("--keep-png", action="store_true", help="Keep PNGs after making GIF (default: remove)")
