...
# ----------------------------------------------------------------------
# Station aliases and normalization for JPL Horizons
# ----------------------------------------------------------------------

# Map various human-readable names / MPC codes to canonical site labels.
# You can extend this if you add more sites.
_ALIASES = {
    # La Palma / GOTO-North
    "La Palma": "La Palma",
    "la palma": "La Palma",
    ...
    # Siding Spring / GOTO-South
    "Siding Spring": "Siding Spring",
    "siding spring": "Siding Spring",
...
# Canonical site name -> (canonical label, Horizons location string)
# For now we use geocenter '500@399' as a safe default; you can replace
# it with geodetic coordinates or a specific ground station later.
_SITE_TABLE = {
    "la palma":      ("La Palma",      "R13"),  # TODO: replace with real coords if desired
    "siding spring": ("Siding Spring", "E55"),  # TODO: replace with real coords if desired
    "geocenter":     ("geocenter",     "500@399"),
}


def _norm_site(site_name: str):
    """
    Normalize a site name (La Palma / R13 / Siding Spring / E55 / 500@399 ...)
    to (canonical_label, Horizons location string).

    canonical_label is a human-readable key ("La Palma", "Siding Spring", "geocenter").
    location is the string passed to Horizons(location=...).
    """
    if site_name is None:
        raise ValueError("site_name is None")

    raw = str(site_name).strip()
    # First resolve aliases, e.g. 'R13' -> 'La Palma'
    canon = _ALIASES.get(raw, raw)
    key = canon.lower()

    # known canonical sites
    if key in _SITE_TABLE:
        return _SITE_TABLE[key]

    # allow directly passing a Horizons center string
    if raw == "500@399":
        return ("geocenter", "500@399")

    # everything else is considered unknown
    known_keys = ", ".join(repr(k) for k in _SITE_TABLE.keys())
    raise ValueError(
        f"unknown site: {site_name!r} → normalized {canon!r} (key={key!r}) "
        f"not in {{{known_keys}}}"
    )
...
def _fetch_one_horizons(site_name: str, time_minute_iso: str, target_name: str, max_retries: int = 2, sleep_s: float = 0.8):
    ...
    for id_val, id_type in tries:
        for attempt in range(1, max_retries+1):
            try:
                obj = Horizons(id=id_val, id_type='smallbody', location=loc, epochs=epochs_jd)
                tab = obj.ephemerides()
                df = tab.to_pandas()
                return dict(
...
def refine_ephemerides_per_row(
    df,
    site_col: str = "site",
    ...
    and write refined astrometry columns:

        RA_pre, DEC_pre, RA_rate_pre, DEC_rate_pre

    The function:
      * Uses a cache keyed by (normalized_site, t_minute, target) to avoid
        repeated JPL calls.
      * Skips rows without site or time.
      * Falls back to geocenter (500@399) if Horizons rejects the site location.

    Parameters
    ----------
    df : pandas.DataFrame
        Input table with at least [site_col, time_col] and optionally target_col.
    site_col : str
        Name of the column containing site labels ("La Palma", "R13", "E55", ...).
    time_col : str
        Name of the column containing observation time (parsed by _to_minute_utc_str).
    target_col : str or None
        Column containing the target designation/name. If None, _choose_target_name(row)
        is used as a fallback.
    print_limit : int
        Maximum number of warning/error lines printed.

    Returns
    -------
    df_out : pandas.DataFrame
        A copy of df with RA_pre / DEC_pre / RA_rate_pre / DEC_rate_pre filled
        where JPL succeeded.
    """
    import numpy as _np
    from tqdm import tqdm
    ...
    for idx, row in it:
        # --- extract site and time -----------------------------------------
        if hasattr(row, "get"):
            site_val = row.get(site_col)
            t_raw = row.get(time_col)
        else:
            site_val = row[site_col] if site_col in df_out.columns else None
            t_raw = row[time_col] if time_col in df_out.columns else None

        t_min = _to_minute_utc_str(t_raw)

        if not site_val or not t_min:
            if printed < print_limit:
                print(
                    f"[warn] skip row index={idx}: "
                    f"site={site_val!r}, time={t_raw!r}"
                )
                printed += 1
            fail += 1
            continue
        ...
        if target_col and target_col in df_out.columns:
            v = row[target_col]
            tgt = str(v).strip() if v is not None else None
        if not tgt:
        ...
        if not tgt:
            if printed < print_limit:
                print(
                    f"[warn] skip row index={idx}: "
                    f"could not determine target name"
                )
                printed += 1
            fail += 1
            continue

        # Use the raw site string as cache key; normalization is inside _norm_site
        site_raw = str(site_val).strip()
        key = (site_raw, t_min, tgt)

        if key not in cache:
            # First attempt: use the site as given (normalized via _norm_site)
            try:
                cache[key] = _fetch_one_horizons(site_raw, t_min, tgt)
            except Exception as e:
                msg = str(e)

                # If Horizons complains about the location/coordinate triplet,
                # try again with geocenter (500@399) as a fallback.
                if ("Cannot read coordinate triplet" in msg
                        or "Query failed without known error message" in msg):
                    if printed < print_limit:
                        print(
                            f"[warn] Horizons rejected site={site_raw!r}, "
                            f"t={t_min}, target={tgt!r}; "
                            f"fallback to geocenter '500@399'"
                        )
                        printed += 1
                    try:
                        cache[key] = _fetch_one_horizons("500@399", t_min, tgt)
                    except Exception as e2:
                        cache[key] = None
                        if printed < print_limit:
                            print(
                                f"[fail] geocenter fallback failed for "
                                f"t={t_min}, target={tgt!r}, err={repr(e2)}"
                            )
                            printed += 1
                else:
                    cache[key] = None
                    if printed < print_limit:
                        print(
                            f"[fail] site={site_raw}, t={t_min}, "
                            f"target={tgt!r}, err={repr(e)}"
                        )
                        printed += 1
        ...
        # --- write JPL results back into df_out ----------------------------
    ...
    print(f"[JPL] {ok} rows succeeded, {fail} rows failed; unique requests={len(cache)}")
...
def main():
    ...
    p.add_argument("--mag-limit", type=str, default="19")
